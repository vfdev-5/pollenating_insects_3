...
class BatchClassifier(object):
    ...
    def fit(self, train_gen_builder):
        ...
        if 'LOCAL_TESTING' in os.environ:
            #
            # I aware that I should not use problem.py and test.csv on remote machine
            # and open test.csv only for my local testing
            #
            ...
            try:
                ...
                gen_valid, _, nb_valid, _ = val_gen_builder.get_train_valid_generators(batch_size=batch_size,
                                                                                       valid_ratio=0.0)
        ...
        self.model.fit_generator(
            ...
            callbacks=get_callbacks(self.model, output_path='submissions/keras_cnns_pretrained'),
    ...
    def _build_model(self):
        ...
        densenet = DenseNetImageNet161(include_top=False, weights='imagenet')
...
def compute_class_undersampled_indices(input_path='data', n_clusters=15, hist_size=100):
    ...
    freq_classes = class_count[class_count > class_count_threshold].sort_values()
    ...
    n_images_per_cluster = int(np.ceil(class_count_threshold / n_clusters))
    output = {}
