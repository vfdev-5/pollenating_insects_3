...
class BatchClassifier(object):
    def __init__(self):
        ...
        if 'LOGS_PATH' in os.environ:
            ...
            now = datetime.now()
            self.logs_path = 'logs_inceptionResNetV2_finetunning_imbalanced7_%s' % now.strftime("%Y%m%d_%H%M")
    ...
    def _compile_model(self, model, lr):
        ...
        model.compile(
            loss=loss, optimizer=Adam(lr=lr, epsilon=1e-3),
...
def get_callbacks(model, logs_path):
    ...
    step_decay_f = lambda epoch: step_decay(epoch, model=model, base=1.5, period=2, verbose=True)
