...
class BatchClassifier(object):
    def __init__(self):
        ...
        if 'LOGS_PATH' in os.environ:
            ...
            now = datetime.now()
            self.logs_path = 'logs_MobileNet_imbalanced_512_%s' % now.strftime("%Y%m%d_%H%M")
    ...
    def _build_model(self):
        mobilenet = MobileNet(input_shape=SIZE + (3, ), include_top=False, weights=None)
        ...
        model = Model(mobilenet.inputs, out)
...
def get_callbacks(model, logs_path):
    ...
    step_decay_f = lambda epoch: step_decay(epoch, model=model, base=1.5, period=7, verbose=True)
