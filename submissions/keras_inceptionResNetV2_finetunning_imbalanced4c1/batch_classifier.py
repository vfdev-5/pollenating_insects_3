...
class BatchClassifier(object):
    def __init__(self):
        ...
        if 'LOGS_PATH' in os.environ:
            ...
            now = datetime.now()
            self.logs_path = 'logs_inceptionResNetV2_finetunning_imbalanced4c1_%s' % now.strftime("%Y%m%d_%H%M")
    ...
    def fit(self, train_gen_builder):
        ...
        # First step : finetunning on rare oversampled classes
        if 'LOCAL_TESTING' in os.environ:
            if 'LOAD_BEST_MODEL' not in os.environ:
                self._finetunning_rare(train_gen_builder, batch_size, valid_ratio, n_epochs_1)
        else:
            self._finetunning_rare(train_gen_builder, batch_size, valid_ratio, n_epochs_1)

        self.model.name = 'InceptionResNetV2_rare'
        load_pretrained_model(self.model, self.logs_path)

        # Second step : finetunning on all classes
        self._finetunning(train_gen_builder, batch_size, valid_ratio, n_epochs_2)
    ...
    def _finetunning(self, train_gen_builder, batch_size, valid_ratio, n_epochs):
        ...
        gen_train, gen_valid, nb_train, nb_valid, _ = \
        ...
        self.model.summary()
        self.model.name = 'InceptionResNetV2'
...
def local_get_rare_train_valid_generators(self, batch_size=256, valid_ratio=0.3):
    ...
    rare_classes = np.where(class_counts < 75)[0]
    train_indices = [index for index in train_indices if self.y_array[index] in rare_classes]

    rs = RandomOverSampler()
    new_train_indices, new_y_array = rs.fit_sample(np.array(train_indices)[:, None],
                                                   self.y_array[train_indices])
    ...
    if valid_indices is not None:
        ...
        rs = RandomOverSampler()
        new_valid_indices, _ = rs.fit_sample(np.array(valid_indices)[:, None],
                                             self.y_array[valid_indices])
        new_valid_indices = new_valid_indices.ravel()
        nb_valid = len(new_valid_indices)
        gen_valid = self._get_generator(indices=new_valid_indices, batch_size=batch_size)
    else:
        nb_valid = None
        gen_valid = None
...
def get_callbacks(model, logs_path):
    ...
    step_decay_f = lambda epoch: step_decay(epoch, model=model, base=1.4, period=4, verbose=True)
