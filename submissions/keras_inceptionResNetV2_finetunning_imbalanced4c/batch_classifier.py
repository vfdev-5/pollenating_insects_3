...
class BatchClassifier(object):
    def __init__(self):
        ...
        if 'LOGS_PATH' in os.environ:
            ...
            now = datetime.now()
            self.logs_path = 'logs_inceptionResNetV2_finetunning_imbalanced4c_%s' % now.strftime("%Y%m%d_%H%M")
    ...
    def _finetunning(self, train_gen_builder, batch_size, valid_ratio, n_epochs):
        ...
        self.model.summary()
        self.model.name = 'InceptionResNetV2_all'
...
def get_callbacks(model, logs_path):
    ...
    step_decay_f = lambda epoch: step_decay(epoch, model=model, base=2.0, period=5, verbose=True)
