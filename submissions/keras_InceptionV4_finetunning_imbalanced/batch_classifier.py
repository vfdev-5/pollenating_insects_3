...
class BatchClassifier(object):
    def __init__(self):
        ...
        if 'LOGS_PATH' in os.environ:
            ...
            now = datetime.now()
            self.logs_path = 'logs_InceptionV4_finetunning_imbalanced_%s' % now.strftime("%Y%m%d_%H%M")
    ...
    def fit(self, train_gen_builder):
        ...
        # Warm-up : Learn only FC layers
        for index, l in enumerate(self.model.layers):
            l.trainable = False
        self.model.layers[-1].trainable = True
        self._compile_model(self.model, lr=0.045)
        self.model.summary()
        ...
        # Finetunning all layer
        for l in self.model.layers:
            l.trainable = True

        self._compile_model(self.model, lr=0.0045)
        self.model.summary()
    ...
    def _compile_model(self, model, lr):
        ...
        model.compile(
            loss=loss, optimizer=RMSprop(lr=lr, epsilon=1.0),
            metrics=['accuracy', f170])

    def _build_model(self):

        # Input size is (299, 299, 3)
        inceptionv4 = create_model(include_top=False, weights='imagenet')
        x = inceptionv4.outputs[0]
        
        # Classification block
        x = GlobalAveragePooling2D(name='avg_pool')(x)
        out = Dense(403, activation='softmax', name='predictions')(x)

        model = Model(inceptionv4.inputs, out)
...
def get_callbacks(model, logs_path):
    ...
    step_decay_f = lambda epoch: step_decay(epoch, model=model, base=2.0, period=1, verbose=True)
